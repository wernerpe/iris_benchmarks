...
def convert_sdf_to_urdf(sdf_file, urdf_file):
    ...
    for sdf_joint in root.findall(".//joint"):
        ...
        # Calculate joint pose relative to parent link
        joint_pose = np.linalg.inv(parent_pose) @ child_pose

        joint_xyz, joint_rpy = matrix_to_pose(joint_pose)[:3], matrix_to_pose(joint_pose)[3:]
        ...
        if sdf_axis is not None:
            ...
            # Transform axis from child link frame to joint frame
            joint_rotation = joint_pose[:3, :3]
            transformed_axis = np.linalg.inv(joint_rotation) @ axis_vec
